...
export const useInfiniteCanvas = () => {
  ...
  //   For performance optimization and animations
  ...
  const onWheel = (e: WheelEvent) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      // Adjust the zoom speed by dividing deltaY by a smaller number
      const adjustedDeltaY = e.deltaY / 2; // Make zoom more responsive
      dispatch(wheelZoom({ deltaY: adjustedDeltaY, originScreen }));
    } else {
      const dx = e.shiftKey ? e.deltaY : e.deltaX;
      const dy = e.shiftKey ? 0 : e.deltaY;
      // Ensure smoother panning by adjusting the values
      dispatch(wheelPan({ dx: -dx * 0.5, dy: -dy * 0.5 }));
    }
  };
  ...
  const onPointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (!isButton) {
      ...
      return; // Don't handle canvas interaction when clicking buttons
    }
      ...
      if (e.button === 0) {
        if (currentTool === "select") {
          ...
          if (hitShape) {
            ...
            Object.keys(selectedShapes).forEach((id) => {
              ...
              if (shape) {
                if (
                  ...
                  // These shapes don't have a `points` array — store their x/y instead
            ...
            // Click an empty space - clear selection and blur any active text inputs
            if (e.shiftKey) {
  ...
  useEffect(() => {
    ...
    // [ ]: THIS IS HOW WE IGNORE THE DEPENDENCY ARRAY :-
    ...
    const handleResizeMove = (e: CustomEvent) => {
      ...
      if (
        ...
        if (actualWidth === 0) {
          ...
          newEndY = newActualX + newActualHeight / 2;
...
export const useFrame = (shape: FrameShape) => {
  ...
  // TIMESTAMP: 7:36:00 watch from here.
